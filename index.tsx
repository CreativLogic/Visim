...
import {
  ...
  TLShapeId,
  TLTextShape,
  // FIX: Import TLVideoShape to correctly type video shapes and their props.
  TLVideoShape,
  toRichText,
  track,
  useEditor,
...
const CustomToolbar = track(() => {
  ...
  return (
    <div className="custom-toolbar">
      ...
      <div className="custom-toolbar-divider" />
      <button
        className="custom-toolbar-button"
        title="Zoom out"
      ...
      <div className="zoom-level-indicator">{zoomLevel}%</div>
      <button
        className="custom-toolbar-button"
        title="Zoom in"
...
const VideoToolbarComponent = track(() => {
  const editor = useEditor();
  const {addToast} = useToasts();
  ...
  // FIX: Add explicit type guard to correctly type `videoShapes` as an array of `TLVideoShape`.
  const videoShapes = editor
    .getSelectedShapes()
    .filter((s): s is TLVideoShape => editor.isShapeOfType(s, 'video'));
  ...
  const getSelectionBounds = () => {
    ...
    const box = new Box(
      fullBounds.x,
      fullBounds.y + fullBounds.height + 10,
      fullBounds.width,
      0,
    );
    return box;
  ...
  const handlePlayPause = () => {
    editor.updateShapes(
      videoShapes.map((shape) => ({
        id: shape.id,
        type: 'video',
        props: {playing: !shape.props.playing},
      })),
    );
  };

  const handleLoopToggle = () => {
    // NOTE: `loop` is not a standard tldraw video shape prop.
    // This may not work without a custom video shape component.
    // FIX: The 'loop' property is not part of the standard TLVideoShapeProps,
    // so we suppress the TypeScript error.
    // @ts-expect-error
    editor.updateShapes(
      videoShapes.map((shape) => ({
        id: shape.id,
        type: 'video',
        props: {loop: !shape.props.loop},
      })),
    );
    addToast({
      title: (videoShapes[0].props as any).loop
        ? 'Looping disabled'
        : 'Looping enabled',
      severity: 'info',
    });
  ...
  const isPlaying = videoShapes.length > 0 && videoShapes[0].props.playing;
  const isLooping =
    videoShapes.length > 0 && (videoShapes[0].props as any).loop;
...
const ContextualToolbarComponent = track(() => {
  ...
  const videoShapes = shapes.filter((shape) =>
  ...
  const hasVideo = videoShapes.length > 0;

  if (
    hasOtherShapes ||
    hasVideo ||
    (textShapes.length === 0 && imageShapes.length === 0)
  )
    return;
  ...
  if (hasText && hasImage) {
    actions.push({
      label: 'Edit image',
      title: 'Edit image from image and text',
      icon: 'genai-generate-image',
      onClick: () => genImageClick(editor),
    });
  }
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const hasVisited = localStorage.getItem('hasVisitedVisim');
    if (!hasVisited) {
  ...
  const handleCloseGettingStarted = () => {
    localStorage.setItem('hasVisitedVisim', 'true');
